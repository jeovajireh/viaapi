...
module.exports = class Sessions {
  ...
  static async start(sessionName) {
    ...
    let existSessDB = await util.getSessionDB(sessionName);
    ...
    if (existSessDB == true) {
      ...
      console.log(
        "❌SESSÃO NÃO SERÁ CRIADA = MOTIVO: A sessão não existe no banco de dados."
      );
      return {
        result: "error",
        state: "NOTFOUND",
    ...
    if (session == false) {
      ...
      ["CONFLICT", "UNPAIRED", "UNLAUNCHED", "UNPAIRED_IDLE"].includes(
        session.state
      )
    ) {
      session.status = "notLogged";
  ...
  static getSession(sessionName) {
    ...
    if (Sessions.sessions) console.log(Sessions.sessions);
  ...
  static async initSession(sessionName) {
    ...
    const client = await wpp.create({
      ...
      statusFind: (statusSession, session_wpp) => {
        console.log("- Status da sessão:", statusSession);
        //return isLogged || notLogged || browserClose || qrReadSuccess || qrReadFail || autocloseCalled || desconnectedMobile || deleteToken
        //Create session wss return "serverClose" case server for close
        console.log("- Session name: ", session_wpp);
    ...
    await session.client.then((client) => {
      ...
      client.onMessage(async (message) => {
        ...
        if (message.type !== "chat") {
          ...
          let imagebase64 =
            "data:" + message.mimetype + ";base64," + base64data;
          ...
          var arqptt = await util.send_postAudio(
            dadosaudio,
            sessionName,
            "arquivo"
          );
      ...
      client.onIncomingCall(async (call) => {
        ...
        client.sendText(
          call.peerJid,
          "Desculpe, ainda não consigo atender chamadas"
        );
      });
      // Listen to ack's
      client.onAck((ack) => {
      ...
      if (session.state == "CONNECTED") {
        var resultSend = await session.client.then(async (client) => {
          if (TypeFile == false) {
            ...
            return await client
              .sendText(soNumeros(params.whatsapp) + "@c.us", params.mensagem)
              .then((result) => {})
              .then(
                (retorno) => {
                  /* sucesso */
            ...
            return await ctrFile
              .formatFilesSend(pars)
              .then(async function (res) {
                ...
                if (res.dirFile) {
                  ...
                  return await client
                    .sendFile(
                      soNumeros(params.whatsapp) + "@c.us",
                      res.dirFile,
                      res.filename,
                      params.mensagem
                    )
  ...
  static async killServiceWorker(sessionName) {
    ...
    if (session) {
      ...
      if (session.state != "CLOSED") {
        if (session.client) {
          var resultkillServiceWorker = await session.client.then(
            async (client) => {
  ...
  static async restartService(sessionName) {
    ...
    if (session) {
      ...
      if (session.state != "CLOSED") {
        if (session.client) {
          var resultrestartService = await session.client.then(
            async (client) => {
  ...
  static async getHostDevice(sessionName) {
    ...
    if (session) {
      ...
      if (session.state != "CLOSED") {
        if (session.client) {
          var resultgetHostDevice = await session.client.then(
            async (client) => {
          ...
          if (retorno.id == "1" && retorno.connected == "1") {
            var errosn = "nao";
            var logado = "sim";
            var pronto = "sim";
          } else {
  ...
  static async getBatteryLevel(sessionName) {
    ...
    if (session) {
      ...
      if (session.state != "CLOSED") {
        if (session.client) {
          var resultgetBatteryLevel = await session.client.then(
            async (client) => {
  ...
  static async getAuth(sessionName) {
    ...
    if (session.qrcode == null) {
      var sucesso = "nao";
      var qrauth = "-";
      var sobre_o_erro = "não startado";
    } else {
      var sucesso = "sim";
      var sobre_o_erro = "sem erros";
      qrauth = session.qrcode;
    }
    ...
    if (session.state == "CONNECTED") {
      ...
      if (whatsapp != "" && sessionName != "") {
        ...
        var perfilImg = await session.client.then(async (client) => {
          ...
          return await client
            .getProfilePicFromServer(soNumeros(whatsapp) + "@c.us")
            .then((result) => {
              return result;
            })
            .catch((erro) => {
              console.error("Error when sending: ", erro); //return object error
              console.log(
                "👉Erro ao consultar a imagem de pergil do contato!",
                erro
              );
              return false;
            });
        ...
        var res = {
          result: false,
          erro: " erro ao enviar o post ao endpoint.",
        };
        ...
        if (perfilImg) {
          ...
          console.log(
            "👍Consulta de imagem de perfil do contato, enviado com êxito." +
              whatsapp +
              ".",
            res,
            params
  ...
  static async sendText(sessionName, number, text) {
    console.log(
      "- Enviando menssagem de texto! " + text + number + sessionName
    );
    var session = Sessions.getSession(sessionName);
    if (session) {
      if (session.state == "CONNECTED") {
  ...
  static async sendTextMult(
    ...
    if (session) {
      if (session.state == "CONNECTED") {
        //
        var folderName = fs.mkdtempSync(
          path.join(os.tmpdir(), session.name + "-")
        );
        ...
        for (var i in arrayNumbers) {
          ...
          if (numero != null || numero != "") {
            var resultsendTextMult = await session.client.then(
              async (client) => {
                // Send basic text
                return await client
                  .sendText(soNumeros(numero) + "@c.us", msgtxtmass)
                  .then((result) => {
    ...
    if (session) {
      if (session.state == "CONNECTED") {
        var resultsendImage = await session.client.then(async (client) => {
          ...
          console.log(filePath);
          return await client
            .sendImage(number + "@c.us", filePath, fileName, caption)
            .then((result) => {
              //console.log('Result: ', result); //return object success
              return result;
            })
    ...
    if (session) {
      if (session.state == "CONNECTED") {
        var resultsendImage = await session.client.then(async (client) => {
          ...
          //
          var arrayNumbers = fs
            .readFileSync(filePathContato, "utf-8")
            .toString()
            .split(/\r?\n/);
          for (var i in arrayNumbers) {
            //console.log(arrayNumbers[i]);
    ...
    if (session) {
      if (session.state == "CONNECTED") {
        var resultsendImage = await session.client.then(async (client) => {
          ...
          console.log(filePath);
          return await client
            .sendImage(number + "@g.us", filePath, fileName, caption)
            .then((result) => {
              //console.log('Result: ', result); //return object success
              return result;
            })
    ...
    if (session) {
      if (session.state == "CONNECTED") {
        var resultSendFile = await session.client.then(async (client) => {
          ...
          console.log(filePath);
          return await client
            .sendFile(number + "@c.us", filePath, fileName, caption)
            .then((result) => {
              //console.log('Result: ', result); //return object success
              return result;
            })
  ...
  static async getSessionTokenBrowser(sessionName) {
    ...
    if (session) {
      if (session.state == "CONNECTED") {
        var resultgetSessionTokenBrowser = await session.client.then(
          async (client) => {
            return await client
  ...
  static async loadAndGetAllMessagesInChat(sessionName, numero) {
    ...
    if (session) {
      if (session.state == "CONNECTED") {
        var resultloadAndGetAllMessagesInChat = await session.client.then(
          async (client) => {
            return await client
              .loadAndGetAllMessagesInChat(soNumeros(numero) + "@g.us")
              .then((result) => {
                //console.log('Result: ', result); //return object success
                return result;
              })
    ...
    if (session) {
      if (session.state == "CONNECTED") {
        var resultgetStatus = await session.client.then(async (client) => {
          return await client
            .getStatus(soNumeros(numero) + "@c.us")
            .then((result) => {
              //console.log('Result: ', result); //return object success
              return result;
            })
  ...
  static async getNumberProfile(sessionName, numero) {
    ...
    if (session) {
      if (session.state == "CONNECTED") {
        var resultgetNumberProfile = await session.client.then(
          async (client) => {
            return await client
              .getNumberProfile(soNumeros(numero) + "@c.us")
              .then((result) => {
                //console.log('Result: ', result); //return object success
                return result;
              })
  ...
  static async getAllUnreadMessages(sessionName) {
    ...
    if (session) {
      if (session.state == "CONNECTED") {
        var resultgetAllUnreadMessages = await session.client.then(
          async (client) => {
            return await client
  ...
  static async getProfilePicFromServer(sessionName, numero) {
    ...
    if (session) {
      if (session.state == "CONNECTED") {
        var resultgetProfilePicFromServer = await session.client.then(
          async (client) => {
            return await client
              .getProfilePicFromServer(soNumeros(numero) + "@g.us")
              .then((result) => {
                //console.log('Result: ', result); //return object success
                return result;
              })
  ...
  static async checkNumberStatus(sessionName, numero) {
    ...
    if (session) {
      if (session.state == "CONNECTED") {
        var resultcheckNumberStatus = await session.client.then(
          async (client) => {
            return await client
              .checkNumberStatus(soNumeros(numero) + "@c.us")
              .then((result) => {
                //console.log('Result: ', result); //return object success
                return result;
              })
  ...
  //
  static async checkNumberStatusMult(
    sessionName,
    base64Data,
    mimetype,
    originalname
  ...
  static async getGroupMembers(sessionName, groupId) {
    ...
    if (session) {
      if (session.state == "CONNECTED") {
        var resultgetGroupMembers = await session.client.then(
          async (client) => {
            return await client
              .getGroupMembers(groupId + "@g.us")
              .then((result) => {
                //console.log('Result: ', result); //return object success
                return result;
              })
  ...
  static async getGroupMembersIds(sessionName, groupId) {
    ...
    if (session) {
      if (session.state == "CONNECTED") {
        var resultgetGroupMembersIds = await session.client.then(
          async (client) => {
            return await client
              .getGroupMembersIds(groupId + "@g.us")
              .then((result) => {
                //console.log('Result: ', result); //return object success
                return result;
              })
  ...
  static async getGroupInviteLink(sessionName, groupId) {
    ...
    if (session) {
      if (session.state == "CONNECTED") {
        var resultgetGroupInviteLink = await session.client.then(
          async (client) => {
            return await client
              .getGroupInviteLink(groupId + "@g.us")
              .then((result) => {
                //console.log('Result: ', result); //return object success
                return result;
              })
    ...
    if (session) {
      if (session.state == "CONNECTED") {
        var resultgetGroupInviteLink = await session.client.then(
          async (client) => {
            return await client
              .createGroup(groupId, ["111111111111@c.us", "222222222222@c.us"])
              .then((result) => {
                //console.log('Result: ', result); //return object success
                return result;
              })
  ...
  static async removeParticipant(sessionName, groupoid, contato) {
    ...
    if (session) {
      if (session.state == "CONNECTED") {
        var resultremoveParticipant = await session.client.then(
          async (client) => {
            return await client
              .removeParticipant(groupoid + "@g.us", contato + "@c.us")
              .then((result) => {
                //console.log('Result: ', result); //return object success
                return result;
              })
    ...
    if (session) {
      if (session.state == "CONNECTED") {
        var resultaddParticipant = await session.client.then(async (client) => {
          return await client
            .addParticipant(groupoid + "@g.us", contato + "@c.us")
            .then((result) => {
              //console.log('Result: ', result); //return object success
              return result;
            })
  ...
  static async promoteParticipant(sessionName, groupoid, contato) {
    ...
    if (session) {
      if (session.state == "CONNECTED") {
        var resultpromoteParticipant = await session.client.then(
          async (client) => {
            return await client
              .promoteParticipant(groupoid + "@g.us", contato + "@c.us")
              .then((result) => {
                //console.log('Result: ', result); //return object success
                return result;
              })
  ...
  static async demoteParticipant(sessionName, groupoid, contato) {
    ...
    if (session) {
      if (session.state == "CONNECTED") {
        var resultdemoteParticipant = await session.client.then(
          async (client) => {
            return await client
              .demoteParticipant(groupoid + "@g.us", contato + "@c.us")
              .then((result) => {
                //console.log('Result: ', result); //return object success
                return result;
              })
    ...
    if (session) {
      if (session.state == "CONNECTED") {
        var resultgetGroupAdmins = await session.client.then(async (client) => {
          return await client
            .getGroupAdmins(groupoid + "@g.us", contato + "@c.us")
            .then((result) => {
              //console.log('Result: ', result); //return object success
              return result;
            })
  ...
  static async getGroupInfoFromInviteLink(sessionName, groupoid, contato) {
    ...
    if (session) {
      if (session.state == "CONNECTED") {
        var resultgetGroupInfoFromInviteLink = await session.client.then(
          async (client) => {
            return await client
              .createGroup(groupoid + "@g.us", contato + "@c.us")
              .then((result) => {
                //console.log('Result: ', result); //return object success
                return result;
              })
    ...
    if (session) {
      if (session.state == "CONNECTED") {
        var resultjoinGroup = await session.client.then(async (client) => {
          return await client
            .joinGroup(groupoid + "@g.us", contato + "@c.us")
            .then((result) => {
              //console.log('Result: ', result); //return object success
              return result;
            })
